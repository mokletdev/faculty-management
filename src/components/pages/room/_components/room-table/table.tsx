...
export const RoomTable: FC<{
  roomData: PaginationResult<Room>;
}> = ({ roomData }) => {
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="text-muted-foreground flex-1 text-sm">
          ...
          {roomData.data.length > 0
            ? (roomData.metadata.currentPage - 1) * roomData.metadata.pageSize +
              1
            : 0}{" "}
          to{" "}
          {Math.min(
            roomData.metadata.currentPage * roomData.metadata.pageSize,
            roomData.metadata.totalCount,
          )}{" "}
          of {roomData.metadata.totalCount} rooms
        </div>
        <div className="space-x-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() => handlePageChange(page + 1)}
            disabled={page >= roomData.metadata.totalPages}
          >
            Next
          </Button>
        </div>
